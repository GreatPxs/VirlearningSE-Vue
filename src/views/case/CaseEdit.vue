...
onBeforeMount(() => {
  editId.value = parseInt(route.params.id.toString())
  console.log(editId.value)
  getCaseInfoById()
})

//获取待编辑病例信息
const getCaseInfoById = async () => {
  const data = await getCaseById(editId.value).then((res) => {
    //获取失败
...
const onSubmit = async () => {
  //修改提交
  const data = await editCase(form.value).then((res) => {
    console.log(res)
    //修改失败
    if (res.data.resultCode !== 200) {
      console.log(res.data)
      ElMessage.error('修改病例失败')

      throw new Error('修改病例失败')
    }
    return res
  })

  console.log(data)
  ElMessage.success('修改病例成功')
  ...
  <el-page-header @back="$router.go(-1)">
    <template #content>
      <div class="flex items-center">
        <span class="text-large font-600 mr-3"> 修改病例 </span>
      </div>
    </template>
  </el-page-header>
      ...
      <div v-show="currentStep === 0">
        ...
        <el-form-item label="疾病名称">
          <el-cascader
            ...
            v-model="form.name"
            :placeholder="form.name ? form.name : 'Select'"
          />
        </el-form-item>
